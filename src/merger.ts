...
`;
  ...
  /**
   * Extrai comentários do usuário (comentários que não são do header padrão)
   */
  private extractUserComments(content: string): string[] {
    ...
    for (const line of lines) {
      ...
      // Após o header, considera tudo como conteúdo do usuário
      if (headerFound) {
        inUserSection = true;
      }
      
      // Comentários do usuário (linhas que começam com # e não são padrão)
      if (inUserSection && line.trim().startsWith('#') && 
  ...
  private combineContent(
    ...
    // Adiciona comentários do usuário se existirem
    if (userComments.length > 0) {
      parts.push('# User comments:');
      parts.push(...userComments);
      parts.push('');
    }
